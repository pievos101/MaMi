...
for(xx in 1:n_iter){
    ...
    res = mami_py(train, test, train_labels_py, k1=as.integer(3), 
                        k2=as.integer(1), dmatrix=D1)
    })
    
    MAMI_k3_time = system.time({
    res = mami_py(train, test, train_labels_py, k1=as.integer(3), 
                        k2=as.integer(10), dmatrix=D1)
    })
    MAMI_k5_time = system.time({
    res = mami_py(train, test, train_labels_py, k1=as.integer(3), 
                        k2=as.integer(50), dmatrix=D1)
    })
    MAMI_k10_time = system.time({
    res = mami_py(train, test, train_labels_py, k1=as.integer(3), 
                        k2=as.integer(100), dmatrix=D1)
    })
    
    
    KNN_time = system.time({
        my_knn_py(train, test, train_labels_py, k = as.integer(3), 
